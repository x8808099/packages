function retVal = bpf2matVolts(filename,drOUT,doEEG,doTetrodeData,doTetrodeTS,doPosition,doSync,doInput)
%converts BPF into Matlab file
% (C) Dino Dvorak 2015
% dino@indus3.net
%
%fileName - input filename
%doEEG - 0(don't process EEG), 1(process EEG)
%doTetrodeData - exports tetrode waveforms
%doTetrodeTS - exports tetrode timestamps (of spikes)
%doPosition - exports XY
%doSync - exports sync signal
%doInput - exports I/O
%example bpf2matVolts('data.bpf','OUT/',1,0,0,0,0,0);

%changes
%25.11.2012 modified tetrode voltage decoding
%11.9.2013 modified EEG voltage decoding to U = 1.5*voltage16bit/32768/gain
%9.4.2015 fixed XY to XYZ position, split doTetrode to doTetrodeData and
%doTetrodeTS
%20.5.2015 changed strsplit to Matlab's syntax
...
fileID = 1;
files = dir([drIn '*.bpf']);
filename = [drIn files(fileID).name];
fileNameOut = [drOUT files(fileID).name];
doEEG = 1;
doTetrodeData = 0;
doTetrodeTS = 1;
doPosition = 0;
...
%do tetrode if tetrode data or timestamps are requested
doTetrode = 0;
if doTetrodeData==1 || doTetrodeTS==1
    doTetrode = 1;
end
...
if waitBarEnable==1
    h = waitbar(0,'Converting to MAT...');

    stepsWB = 100;
    bytesStep = floor((sizeoffile-bytesRead) / stepsWB);
    bytesLastUpdate = 0;
end
...
while bytesRead < sizeoffile
    ...
    if waitBarEnable==1
        bytesUpdate = floor(bytesRead / bytesStep);
        if bytesUpdate ~= bytesLastUpdate
            waitbar(bytesUpdate/stepsWB)
            bytesLastUpdate = bytesUpdate;
        end
    end
...
%TETRODE DATA
if doTetrodeData == 1
    dr = [drOUT 'TETRODE_DATA/'];
    if exist(dr,'dir') == 0; mkdir(dr); end;
    save ([dr fnOut], 'tetrodeData', 'tetrodeFS', 'tetrodeTimestamp','tetrodeSamplesPerBlock','tetrodeChannel','tetrodeUnit');
end
...
if doTetrodeTS == 1    
    dr = [drOUT 'TETRODE_TS/'];
    if exist(dr,'dir') == 0; mkdir(dr); end;
    save ([dr fnOut],'tetrodeFS', 'tetrodeTimestamp','tetrodeChannel','tetrodeUnit');
end
...
if doSync == 1
    dr = [drOUT 'SYNC/'];
    if exist(dr,'dir') == 0; mkdir(dr); end;
    save ([dr fnOut], 'syncData','syncTimeStamp');
end
...
if doEEG == 1
    dr = [drOUT 'MAT/'];
    if exist(dr,'dir') == 0; mkdir(dr); end;
    save ([dr fnOut], 'eegData', 'eegTimeStamp', 'eegFS');
end
...
if doInput == 1
    dr = [drOUT 'INPUT/'];
    if exist(dr,'dir') == 0; mkdir(dr); end;
    save ([dr fnOut], 'inputTimeStamps', 'inputData');
...
if doPosition == 1
    dr = [drOUT 'POS/'];
    if exist(dr,'dir') == 0; mkdir(dr); end;
    save ([dr fnOut], 'roomTimeStamps', 'roomXY','arenaTimeStamps','arenaXY');
end
